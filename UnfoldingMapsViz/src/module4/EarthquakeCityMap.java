...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		text("Ocean Quake", 70, 155);
		
		text("Shallow", 70, 210);
		text("Intermediate", 70, 235);
		text("Deep", 70, 260);
		
		text("Past day", 70, 285);
	}
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> landQuakeCounter = new HashMap<String, Integer>();
		int oceanQuakeCounter = 0;
		for (Marker m : quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker)m;
			if (em.isOnLand()) {
				String country = em.getProperty("country").toString();
				if (!landQuakeCounter.containsKey(country)) {
					landQuakeCounter.put(country, 1);
				}
				else {
					int prevVal = landQuakeCounter.get(country);
					landQuakeCounter.put(country, prevVal + 1);
				}
			}
			else {
				oceanQuakeCounter++;
		...
		for (String country: landQuakeCounter.keySet()) {
			System.out.println(country + ": " + landQuakeCounter.get(country));
		}
